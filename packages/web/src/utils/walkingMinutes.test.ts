...
describe('walkingMinutes', () => {
  ...
  it('rounds up so 81 m is 2 minutes (conservative pace)', () => {
    // Prefer over-estimating to under-estimating: the user planning to
    // catch a bus would rather arrive early than late.
...
describe('formatWalkingMinutes', () => {
  it('renders "<1 min walk" for distances under one minute', () => {
    expect(formatWalkingMinutes(20)).toBe('<1 min walk');
  });

  it('renders "1 min walk" at exactly 80 m', () => {
    expect(formatWalkingMinutes(80)).toBe('1 min walk');
  });

  it('renders "5 min walk" for a 400 m walk', () => {
    expect(formatWalkingMinutes(400)).toBe('5 min walk');
  });

  it('renders "<1 min walk" for 0 m (don\'t claim instant)', () => {
    expect(formatWalkingMinutes(0)).toBe('<1 min walk');
  });
});
